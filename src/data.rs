...
pub struct Boei {
    ...
    // Note: CSV column names are misleading - "Lat_min" contains longitude, "Long_min" contains latitude
...
mod tests {
    ...
    fn test_coordinate_parsing() {
        ...
        assert!(result.is_ok(), "Failed to parse coordinates: {:?}", result.err());
        ...
        assert!((boei.long.unwrap() - expected_long).abs() < 0.001, 
                "Longitude parsing incorrect. Expected: {}, Got: {}", expected_long, boei.long.unwrap());
        
        // Check latitude: 53° 5,020' = 53 + 5.020/60 = 53.08367... (latitude)
        let expected_lat = 53.0 + 5.020 / 60.0;
        assert!((boei.lat.unwrap() - expected_lat).abs() < 0.001, 
                "Latitude parsing incorrect. Expected: {}, Got: {}", expected_lat, boei.lat.unwrap());
    }
    ...
    fn test_coordinate_parsing_edge_cases() {
        ...
        for (input, expected) in test_cases {
            ...
            assert!((parsed - expected).abs() < 0.001, 
                    "Parsing '{}' failed. Expected: {}, Got: {}", input, expected, parsed);
        }
    }
